...
df = pd.read_csv('berlin52.tsp', skiprows=6, header=None)
city = np.array(df[0][0:len(df)-1])  # 最后一行为EOF，不读入
data_initial = []
for i in city:
    data_initial.append(i.split())
city_name = [i[0] for i in data_initial]
city_x = [float(i[1]) for i in data_initial]
city_y = [float(i[2]) for i in data_initial]
city_location = list(zip(city_x, city_y))
print(city_location)
coordinates = np.array(city_location)
...
while iter < itermax:
    ...
    for i in range(numant):
        ...
        for j in range(1, numc.ity):  # 循环numcity-1次，访问剩余的所有numcity-1个城市
            # j=1
            # 每次用轮盘法选择下一个要访问的城市
...
'''
plt.plot(coordinates[:, 0], coordinates[:, 1], 'r.')
plt.plot(coordinates[:, 0], coordinates[:, 1], marker='>', color='r')
'''
plt.xlim([-100, 2000])
# x范围
plt.ylim([-100, 1500])
